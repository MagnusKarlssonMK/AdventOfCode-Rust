...
impl Board {
    ...
    fn get_bingoround_and_score(&self, drawmap: &HashMap<usize, usize>) -> (usize, usize) {
        let bingo_drawidx = self.rows
            .iter()
            .chain(self.cols.iter())
            .map(|row| row
                .iter()
                .map(|v| drawmap.get(v).unwrap())
                .max().unwrap())
            .min().unwrap();
...
impl InputData {
    fn parse_input(input: &str) -> Self {
        ...
        let draw: Vec<usize> = blocks.next().unwrap().split(',').map(|c| c.parse().unwrap()).collect();
        ...
        let boards: Vec<Board> = blocks
                        .map(Board::from_str)
                        .collect();
        Self { draw, drawmap, boards }
    }

    fn solve_parts1_2(&self) -> (usize, usize) {
        let mut results: Vec<(usize, usize)> = self.boards.iter().map(|board| board.get_bingoround_and_score(&self.drawmap)).collect();
        results.sort_unstable_by(|round, score| round.partial_cmp(score).unwrap());
        (self.draw[results.first().unwrap().0] * results.first().unwrap().1, self.draw[results.last().unwrap().0] * results.last().unwrap().1)
    }
}
...
mod tests {
    ...
    fn part1_2_example_1() {
        let testdata = String::from("7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1
