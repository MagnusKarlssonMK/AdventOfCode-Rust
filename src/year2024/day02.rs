...
impl Report {
    ...
    fn get_safety_level(&self) -> SafetyLevel {
        fn is_safe(levels: &[usize]) -> bool {
            let diffs: Vec<(usize, Ordering)> = levels.windows(2)
                .map(|w| (w[1].abs_diff(w[0]), w[1].cmp(&w[0])))
                .collect();
            diffs.iter().all(|&(v, o)|
                (1..=3).contains(&v) && o == diffs.first().unwrap().1)
        }
        if is_safe(&self.levels) {
            SafetyLevel::Safe
        } else {
            for n in 0..self.levels.len() {
                let remove_one: Vec<usize> = self.levels.iter()
                    .enumerate()
                    .filter(|(i, _)| *i != n)
                    .map(|(_, level)| *level)
                    .collect();
...
impl InputData {
    ...
    fn solve(&self) -> (usize, usize) {
        let safety: Vec<SafetyLevel> = self.reports.iter()
            .map(|r| r.get_safety_level())
            .collect();
        let safe: usize = safety.iter()
            .filter(|s| **s == SafetyLevel::Safe)
            .count();
        let almost_safe: usize = safety.iter()
